...
export const getVisitStatus = async (sectionId: number) => {
  try {
    ...
    const response = await apiCall.get(`${BASE_URL}/sections/${sectionId}/current`);
    console.log('API 응답:', response);
    
    // 응답이 비어있거나 next_num이 0인 경우, 대기 중인 첫 번째 방문을 조회
    if (!response.next_num) {
      try {
        const pendingVisit = await apiCall.get(`${BASE_URL}/sections/${sectionId}/pending/first`);
        console.log('대기 중인 첫 번째 방문:', pendingVisit);
        
        if (pendingVisit && pendingVisit.data) {  // data 객체 확인
          return {
            ...response,
